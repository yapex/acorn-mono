...
#
# 格式类型定义：
#   percentage  - 值域 0-100，显示时直接加 % 后缀（不乘100）
#                 示例: roe=38.43 → "38.43%"
#   ratio       - 实数，显示时不加 %，保留 2 位小数
#                 示例: current_ratio=4.45 → "4.45"
#   absolute    - 金额类，显示时按数量级加 亿/万 后缀
#                 示例: total_assets=2.989e12 → "2989.45亿"
#   yoy         - 同比增长率，显示时加 % 后缀
#                 示例: revenue_yoy=15.0 → "15.00%"
#   market      - 估值指标，显示时不加 %（如 PE=25.0）
#
# 注意：
#   - 数据库存储的值就是最终要显示的值，format_type 只决定是否加 % 后缀
#   - 计算器的格式类型由计算器脚本声明（FORMAT_TYPE / FORMAT_TYPES）
