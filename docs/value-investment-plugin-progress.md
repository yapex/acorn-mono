...
def vi_fetch_financials(...) -> dict        # 获取财务报表数据
def vi_fetch_indicators(...) -> dict        # 获取财务指标
def vi_fetch_market(...) -> dict            # 获取市场数据
...
- 财务指标：roe, roa, gross_margin, net_profit_margin, ...
- 市场数据：market_cap, pe_ratio, pb_ratio, ...
...
```
Total fields: 76 (ifrs: 38 + extension: 38)
Provider fields: 78 (tushare)

Pluggy integration: ✅
  - vi_markets: [['A']]
  - vi_supported_fields: 78 fields
```
...
pm.load_setuptools_entrypoints('value_investment.providers')
```
...
- [ ] calculators - 计算器插件机制
- [ ] vi_cli - 命令行工具
- [ ] query 命令完整实现（使用 provider_tushare）
- [ ] acorn-agent 集成
